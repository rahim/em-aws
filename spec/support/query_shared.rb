shared_examples "an AWS Query" do
  ...
  context "making requests", :mock do
    ...
    it "adds a version" do
      ...
      expect(WebMock).to have_requested(:post, @url).with(body: hash_including('Version' => subject.class::API_VERSION))
    end
    
    it "adds a timestamp if there was none" do
      event {subject.call :dummy_action}
      expect(WebMock).to have_requested(:post, @url).with(body: hash_including('Timestamp' => '2011-07-09T20:30:03Z'))
    end
    
    it "keeps the timestamp intact if there was one" do
      event {subject.call :dummy_action, timestamp: '2012-02-06T19:33:33Z'}
      expect(WebMock).to have_requested(:post, @url).with(body: hash_including('Timestamp' => '2012-02-06T19:33:33Z'))
    end
      
    it "also passes any parameters" do
      event {subject.call :DummyAction, ThisThing: 'is cool!'}
      expect(WebMock).to have_requested(:post, subject.url).with(body: hash_including({
        'ThisThing' => 'is cool!', 
        'Action' => 'DummyAction'}))
    end
    ...
    it "doesn't sign if no access key was given" do
      ...
      expect(WebMock).not_to have_requested(:post, @url).with(body: hash_including({
        'SignatureVersion' => '2'
      }))
    ...
    it "handles GET requests too" do
      ...
      expect(WebMock).to have_requested(:get, subject.url).with(query: hash_including({
        'Action' => 'DummyAction',
        'SomeParam' => 'foo',
        'AWSAccessKeyId' => 'FAKE_KEY'
      }))
  ...
  context "handling responses", :mock do
    ...
    it "takes a block for a callback" do
      ...
      expect(@response).to be_kind_of(EM::AWS::Response)
      expect(@response.answer_one).to be == 'foo'
    end
    
    it "returns the request when called within EventMachine" do
      litmus = nil
      event {litmus = subject.dummy_action}
      expect(litmus).to be_instance_of(EM::AWS::Request)
      expect(litmus.response.answer_one).to be == 'foo'
    end
    
    it "returns the request when called synchronously" do
      litmus = subject.dummy_action zoo: 'zar'
      expect(litmus).to be_instance_of(EM::AWS::Request)
      expect(litmus['AnswerTwo']).to be == 17
    end
    ...
    it "raises an exception when called synchronously" do
      ...
      expect(->{subject.dummy_action floo: 'flar'}).to raise_error(EM::AWS::Query::QueryError, /DummyFailure/)
    end
  end
